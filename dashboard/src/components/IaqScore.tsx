  ...
  return (
    <div className={cn('rounded-2xl p-6 text-white flex flex-col gap-3', band.color)}>
      <div className="flex items-start justify-between">
        <div>
          <p className="text-sm font-medium uppercase tracking-widest opacity-80">Air Quality Index</p>
          <p className="text-8xl font-black leading-none mt-1">{Math.round(iaq)}</p>
        </div>
        <div className="text-right">
          <p className="text-2xl font-bold">{band.label}</p>
          <p className="text-sm opacity-80 mt-1 max-w-[180px] leading-snug">{band.description}</p>
        </div>
      </div>
      ...
      <p className="text-xs opacity-70">
        Sensor: {IAQ_ACCURACY_LABEL[accuracy] ?? 'Unknown'}
      </p>
    </div>
  )
}
