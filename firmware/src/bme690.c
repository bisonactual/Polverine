...
static void bsec_state_save(const uint8_t *buf, uint32_t len)
{
    ...
    if (err == ESP_OK) {
        ESP_LOGI(TAG, "BSEC state saved (%lu bytes)", (unsigned long)len);
    } else {
        ESP_LOGW(TAG, "BSEC state save failed: %s", esp_err_to_name(err));
    }
}
...
static void output_ready(outputs_t *out)
{
    ...
    s_latest.pressure = out->raw_pressure / 100.0f;
...
{
    (void)arg;
    ESP_LOGI(TAG, "delaying BSEC startup to avoid sensor bring-up contention");
...
bool bme690_init(void)
{
    ...
    if (err == ESP_ERR_NVS_NO_FREE_PAGES || err == ESP_ERR_NVS_NEW_VERSION_FOUND) {
        ...
        err = nvs_flash_init_partition(BSEC_NVS_PARTITION);
    }
    if (err != ESP_OK) {
    ...
    if (s_data_mutex == NULL) {
        ESP_LOGE(TAG, "failed to create BME690 mutex");
        return false;
    }
...
{
    if (s_dev_handle == NULL || s_data_mutex == NULL) {
        ESP_LOGW(TAG, "BME690 task not started because init did not complete");
        return false;
    }

    BaseType_t ok = xTaskCreatePinnedToCore(bsec_task, "bsec", 12288, NULL, 3, NULL, 1);
    if (ok != pdPASS) {
